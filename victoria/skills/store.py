...
class SkillStore:
    ...
    def list(self) -> list[Skill]:
        ...
        for path in sorted(self.dir.glob("*.md")):
            try:
                skill = self._parse(path.read_text(encoding="utf-8"), fallback_name=path.stem)
                skill.path = path
                skills.append(skill)
            except Exception:
    ...
    # -- writes ----------------------------------------------------------- #
    def save(self, name: str, description: str, instructions: str) -> Skill:
        skill = Skill(name=name.strip(), description=description.strip(),
                      instructions=instructions.strip())
        path = self.dir / f"{slugify(name)}.md"
